...
/** Seller inbox / bell refresh (from upsertSellerNotification). */
export function onSellerNotification(getToken, handler) {
  const s = getOrderSocket(getToken);
  if (!s || typeof handler !== "function") return () => {};
  s.on("seller_notification", handler);
  s.on("admin_notification", handler);
  return () => {
    s.off("seller_notification", handler);
    s.off("admin_notification", handler);
  };
}
