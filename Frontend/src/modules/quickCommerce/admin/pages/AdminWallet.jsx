...
const AdminWallet = () => {
            ...
            if (summaryRes.data.success || ledgerRes.data.success) {
                ...
                setWalletData({
                    stats: {
                        ...
                        totalPackingFee: summary.totalPackingFee || 0,
                        totalReturnPickupFees: summary.totalReturnPickupFees || 0,
                        totalAdminEarning: summary.totalAdminEarning || 0,
                        totalSellerCommission: summary.totalSellerCommission || 0,
                        totalAdminCouponDiscount: summary.totalAdminCouponDiscount || 0,
                        totalSellerCouponDiscount: summary.totalSellerCouponDiscount || 0,
    ...
    const stats = [
        {
            ...
            value: `₹${Number(walletData.stats?.totalAdminEarning || 0).toLocaleString()}`,
            description: Number(walletData.stats?.totalAdminEarning || 0) < 0
                ? 'Negative after admin coupons / GST refunds — recovers as fees grow'
                : 'Fee + GST + commission + delivery margin − admin coupons − return GST',
            icon: IndianRupee,
            color: 'purple',
            bg: 'bg-purple-50',
        ...
        },
        {
            ...
            value: `₹${Number(walletData.stats?.totalSellerCommission || 0).toLocaleString()}`,
            description: 'Included in admin earning',
            icon: CreditCard,
            color: 'indigo',
            bg: 'bg-indigo-50',
        ...
        },
        {
            ...
            value: `- ₹${Number(walletData.stats?.totalAdminCouponDiscount || 0).toLocaleString()}`,
            description: 'Cut from admin earnings only',
            icon: ArrowDownLeft,
            color: 'rose',
            bg: 'bg-rose-50',
        ...
        {
            label: 'Packing Fees (Seller Share)',
            value: `₹${(walletData.stats?.totalPackingFee || 0).toLocaleString()}`,
            description: Number(walletData.stats?.totalReturnPickupFees || 0) > 0
                ? `Gross packing. Net pending deducts ₹${Number(walletData.stats.totalReturnPickupFees).toLocaleString()} return pickup.`
                : 'Stays with sellers (not admin). Return pickup fee later goes to rider.',
            icon: Wallet,
            color: 'emerald',
            bg: 'bg-emerald-50',
        ...
        },
        {
            ...
            description: 'Owed to sellers (packing − return pickup − withdrawals)',
            icon: CreditCard,
            color: 'blue',
            bg: 'bg-blue-50',
    ...
    const handleProcessPayouts = async () => {
        try {
            ...
            const res = await adminApi.processFinancePayouts({
                limit: 100,
                remarks: "Bulk payout processing from ECS panel",
            });
            if (res.data.success) {
