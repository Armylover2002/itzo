        ...
        try {
            ...
            if (response.data.success) {
                ...
                const formatted = dbOrders.map(o => ({
                    ...
                    originalPaidTotal: Number(o.originalPaidTotal || o.amount || o.pricing?.total || 0),
                    refundedAmount: Number(o.refundedAmount || 0),
                    netAfterReturn: Number(
                        o.netAfterReturn ??
                          Math.max(
                            0,
                            Number(o.originalPaidTotal || o.amount || o.pricing?.total || 0) -
                    ...
                    returnStatusLabel: o.returnStatusLabel || '',
                    hasReturn: Boolean(o.hasReturn || o.returnStatus),
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    {order.hasReturn || order.returnStatus ? (
                                                        <Badge className="text-[9px] font-bold bg-amber-50 text-amber-700 border-amber-200 py-0.5 uppercase">
                                                            {order.returnStatusLabel || String(order.returnStatus || 'Return').replace(/_/g, ' ')}
                                                        </Badge>
                                                    ) : null}
                                    ...
                                    <td className="px-4 py-5 text-right">
                                        <div className="flex flex-col items-end">
                                            <span className="text-sm font-black text-slate-900">
                                                ₹{(order.refundedAmount > 0 ? order.netAfterReturn : order.amount).toLocaleString()}
                                            </span>
                                            {order.refundedAmount > 0 ? (
                                                <span className="text-[10px] font-bold text-rose-500 mt-0.5">
                                                    −₹{order.refundedAmount.toLocaleString()} refunded
                                                </span>
                                            ) : (
