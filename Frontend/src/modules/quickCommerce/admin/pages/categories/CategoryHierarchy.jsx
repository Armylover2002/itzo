        ...
        <div className="flex items-center gap-3 overflow-hidden">
          <div
            className={`
                    ...
                    `}>
            {item.iconId && iconComponents[item.iconId] ? (
              <div className="w-5 h-5 text-indigo-600 flex items-center justify-center">
                {(() => {
                  const IconComp = iconComponents[item.iconId];
                  return <IconComp fontSize="small" />;
                })()}
              </div>
            ) : item.iconId && getIconSvg(item.iconId) ? (
              <div
                className="w-5 h-5 text-indigo-600"
                dangerouslySetInnerHTML={{
                  __html: getIconSvg(item.iconId),
                }}
              />
