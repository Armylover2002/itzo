...
import { findCartLineForProduct, applyVariantToProduct, getVariantKey } from '../../utils/productMeta';
import { getFirstSellableVariant, getSelectableVariants, hasVariantPicker } from '../../utils/firstVariantDisplay';
...
import { DEFAULT_PRODUCT_IMAGE, handleProductImageError } from '@/shared/utils/productImage';
...
const ProductDetailSheet = () => {
    ...
    const allImages = useMemo(() => {
        ...
        const imgs = [];
        if (source.mainImage) imgs.push(source.mainImage);
        else if (source.image) imgs.push(source.image);
        if (Array.isArray(source.galleryImages)) {
            imgs.push(...source.galleryImages);
        }
    ...
    const cartProduct = useMemo(() => {
        ...
        if (selectedVariant) {
            return applyVariantToProduct(selectedProduct, selectedVariant);
        }
        if (selectedProduct.selectedVariant) return selectedProduct;
        const first = getFirstSellableVariant(selectedProduct);
        return first ? applyVariantToProduct(selectedProduct, first) : selectedProduct;
    }, [selectedProduct, selectedVariant]);

    const cartItem = useMemo(() => {
        if (!cartProduct) return null;
        return findCartLineForProduct(
            cart,
            cartProduct,
            selectedVariant || cartProduct.selectedVariant,
        );
    }, [cart, cartProduct, selectedVariant]);

    const quantity = cartItem?.quantity ?? 0;
    const cartLineId = cartItem?.id || cartItem?._id || cartProduct?.id || productId;
    const isWishlisted = useMemo(
        () => (selectedProduct ? isVariantWishlisted(baseProductId, selectedVariant) : false),
        [selectedProduct, selectedVariant, isVariantWishlisted, baseProductId],
    );
    ...
    useEffect(() => {
        if (!selectedProduct) return;
        const variants = getSelectableVariants(selectedProduct);
        const savedVariant =
            resolveWishlistVariant(selectedProduct) ||
            getSavedWishlistVariant(selectedProduct.id || selectedProduct._id);
        const initialVariant =
            (savedVariant &&
                variants.find((variant) => getVariantKey(variant) === getVariantKey(savedVariant))) ||
            getFirstSellableVariant(selectedProduct);
        setSelectedVariant(initialVariant || null);
        setActiveImageIndex(0);
        if (productId) fetchReviews(productId);
    }, [selectedProduct, productId, fetchReviews, getSavedWishlistVariant]);

    useEffect(() => {
        setActiveImageIndex(0);
    }, [selectedVariant?.id, selectedVariant?._id, selectedVariant?.sku, allImages]);
    ...
    const toggleWishlist = useCallback(async (e) => {
        ...
        const wishlistProduct = selectedVariant
            ? applyVariantToProduct(selectedProduct, selectedVariant)
            : selectedProduct;
        const wasWishlisted = isVariantWishlisted(baseProductId, selectedVariant);
    ...
    }, [toggleWishlistGlobal, selectedProduct, selectedVariant, isVariantWishlisted, baseProductId, showToast]);

    const handleAddToCart = useCallback(async () => {
        const openingHours =
            selectedProduct?.openingHours ||
            selectedProduct?.seller?.shopInfo?.openingHours ||
            selectedProduct?.seller?.openingHours ||
            "";
        const shopOpen =
            selectedProduct?.isShopOpen != null
                ? Boolean(selectedProduct.isShopOpen)
                : isStoreCurrentlyOpen(openingHours);
        if (!shopOpen) {
            showToast(buildShopClosedMessage(openingHours), "error");
            return;
        }
        if (hasVariantPicker(selectedProduct)) {
            setShowVariantPicker(true);
            return;
        }
        const productForCart = cartProduct || selectedProduct;
        const stock = Number(productForCart?.stock ?? selectedProduct?.stock ?? 0);
        if (stock <= 0) {
    ...
    }, [addToCart, selectedProduct, cartProduct, showToast]);

    const handleIncrement = useCallback(() => {
        const stock = Number((cartProduct || selectedProduct)?.stock ?? 0);
        if (quantity >= stock) {
            showToast(`Only ${stock} items are available in stock.`, "error");
            return;
        }
        updateQuantity(cartLineId, 1);
    }, [updateQuantity, cartLineId, cartProduct, selectedProduct, quantity, showToast]);

    const handleDecrement = useCallback(() => {
        if (quantity === 1) removeFromCart(cartLineId);
        else updateQuantity(cartLineId, -1);
    }, [quantity, removeFromCart, updateQuantity, cartLineId]);
                                    ...
                                    {allImages.length > 1 && (
                                        <div className="flex flex-col gap-2 px-3 py-2 overflow-y-auto no-scrollbar">
                                            {allImages.slice(0, 5).map((img, i) => (
                                                ...
                                                >
                                                    <img src={img || FALLBACK_IMAGE} alt="" loading="lazy" className="w-full h-full object-contain p-1.5 mix-blend-multiply dark:mix-blend-normal" onError={handleProductImageError} />
                                                </motion.button>
                                            ))}
                                        </div>
                                    )}
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="px-5 pt-2 pb-6">
                                ...
                                <h2 className="text-2xl font-black text-[#1A1A1A] dark:text-slate-100 leading-tight mb-1">
                                    {selectedProduct.name}
                                </h2>
                                <div className="flex items-center gap-1.5 mb-4">
                                    ...
                                    <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">
                                        Sold by: <span className="text-[#1A1A1A] dark:text-slate-100">{storeName}</span>
                                    </span>
                                </div>
                            ...
                            <div className="flex flex-col gap-3">
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex flex-col">
                                        ...
                                        <div className="text-2xl font-black text-[#1A1A1A] dark:text-slate-100">₹{currentPrice}</div>
                                    </div>
