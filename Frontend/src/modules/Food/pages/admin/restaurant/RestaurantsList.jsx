      ...
      try {
        ...
        if (rawList.length > 0 || body?.success === true) {
          const mappedRestaurants = rawList.map((restaurant, index) => ({
            ...
            liveTrackingEnabled: Boolean(restaurant.liveTrackingEnabled),
            currentLocation: restaurant.currentLocation || null,
            lastLocationUpdate: restaurant.lastLocationUpdate || null,
                            ...
                            <div className="flex flex-col">
                              ...
                              {restaurant.businessType === "Street Food Vendor" && (
                                <div className="flex flex-col gap-1 mt-1">
                                  <span className="w-fit px-2 py-0.5 rounded text-[10px] font-semibold flex items-center gap-1 bg-emerald-50 text-emerald-700">
                                  ...
                                  {(restaurant.currentLocation?.latitude || restaurant.currentLocation?.coordinates?.length >= 2) && (
                                    <div className="flex items-center gap-1.5 text-[10px] text-slate-500">
                                      {restaurant.lastLocationUpdate && (
                                        <span>Upd: {new Date(restaurant.lastLocationUpdate).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</span>
                                      )}
                                      <a
