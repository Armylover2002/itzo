...
const sellerSchema = new mongoose.Schema(
  {
    ...
    /**
     * Normalized copies of `shopInfo.alternatePhone`. A seller signs in with either of their
     * two numbers, so the alternate needs the same indexed digit forms as the primary.
     */
...
sellerSchema.pre("validate", function normalizeSeller(next) {
  ...
  const alternateDigits = String(this.shopInfo?.alternatePhone || "")
    .replace(/\D/g, "")
    .slice(-15);
  this.alternatePhoneDigits = alternateDigits || undefined;
  this.alternatePhoneLast10 = alternateDigits
    ? alternateDigits.slice(-10)
    : undefined;
...
// Login resolves a seller from either number, and no two sellers may claim the same alternate.
sellerSchema.index({ alternatePhoneLast10: 1 }, { unique: true, sparse: true });
sellerSchema.index({ alternatePhoneDigits: 1 });
