...
export async function softDeleteRestaurant(req, res, next) {
    try {
        const { id } = req.params;
        if (!id || !mongoose.Types.ObjectId.isValid(id)) {
        ...
        const updated = await adminService.softDeleteRestaurant(id);
        if (!updated) return res.status(404).json({ success: false, message: 'Restaurant not found or already deleted' });
        res.status(200).json({ success: true, message: 'Restaurant deleted successfully', data: { restaurant: updated } });
    } catch (error) {
        next(error);
    }
}
...
export async function softDeleteDeliveryPartner(req, res, next) {
    try {
        const { id } = req.params;
        if (!id || !mongoose.Types.ObjectId.isValid(id)) {
        ...
        const updated = await adminService.softDeleteDeliveryPartner(id);
        if (!updated) return res.status(404).json({ success: false, message: 'Delivery partner not found or already deleted' });
        res.status(200).json({ success: true, message: 'Delivery partner deleted successfully', data: { partner: updated } });
    } catch (error) {
        next(error);
    }
}
